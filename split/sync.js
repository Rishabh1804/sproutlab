...
//   sl_sync_dirty      → { [lsKey]: msOfLastLocalWrite }
...
function _syncClearDirty(keys, notAfter, notBefore) {
  var m = _syncLoadDirty();
  var changed = false;
  (keys || []).forEach(function(k) {
  ...
  try { localStorage.setItem(SYNC_LAST_OK_KEY, String(Date.now())); } catch(e) {}
  if (typeof _syncNotifyVisibility === 'function') _syncNotifyVisibility();
}
...
function _syncFlushDirty(hRef) {
  ...
  if (settled.length > 0) _syncClearDirty(settled, startedAt);
...
// "3 changes on this phone haven't reached the cloud since 12 Mar." (PR #265)
