...
- **Phase 3:** wire the manifest into the app `data.js FOOD_EFFECTS` for the Finding-A surfacing. ✅ honey + peanut + tree nut (#187), egg + soy + wheat + sesame + polarity-aware banner (#208). *In progress: the broader food classes — **cow milk + plant milks** brief landed (the first `drink-timing` + `substitute-caveat` records; FOOD_EFFECTS wiring follows the P1c polarity spec, Kael/Vela-reviewed); **fish** brief landed (`allergen-introduce-early` + `choking-by-form`, the new mercury species-selection axis in `safeForm` — uses ESTABLISHED render polarities, no new spec). Manifest now holds **10** records. Then: the choking set.***
