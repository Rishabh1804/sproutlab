...
| cc-026 | global | Per-Province-Layout — Companion specs deploy as Province mirrors. **Amended 2026-06-10 (SproutLab):** Province self-governs its own Companion specs; Codex = record-keeping, not routing/governance. Province wins on collision; the session-start Codex overlay gap-fills only. |
...
- **Province self-governance over Companion specs (Architect, 2026-06-10) — canon-cc-026 amendment.** SproutLab governs its own Companion specs; **Codex is record-keeping for SproutLab, not its routing or governance authority.** Trigger: Province-authored content (Lyra's naming-lore) was being clobbered at session start by the Codex overlay ("Codex wins on name collision"), so the lore read as *missing* from the loaded `lyra` persona even though it was committed in `sproutlab/.claude/agents/lyra.md`. Fix landed in three places: (1) `.claude/hooks/session-start.sh` overlay flipped from clobber (`cp -f`) to **gap-fill** (`[ -e dst ] && continue`) — Province specs win, Codex only adds roles the Province does not ship (e.g. the Consul); (2) `CLAUDE.md` §Companion-Set Invocation Surface reframed — Province-authoritative bodies, Codex reference-only; (3) this record + the cc-026 table row. **canon-cc-010 ("records are Codex") still holds** — that is record-keeping, the role Codex retains; the Chronicler's authored records still persist to Codex. Mirror-header provenance comments (`authored and maintained in Codex`) updated for the `lyra` pair; the remaining Companion headers (maren / kael / vela / ceres / cipher / chronicler + skills) are flagged for a follow-up provenance sweep, non-load-bearing (the hook + CLAUDE.md now carry the authority).
...
**What "Ziva" means + the name not taken.** *Ziva* (Hebrew זִיוָה) means **radiance / light / brightness**. The thread closes on itself: the child named Radiance was born under the constellation that rode *radiant* over her, with Vega — one of the night's most luminous stars — blazing at its crown. Her name and her birth-sky are the same word. **The naming finalists were exactly two — Ziva and Lyra**; the Architect chose *Ziva* and set *Lyra* aside as a daughter's name (it is an established brand in India), so *the name not given to the daughter became the name of her Companion.* Lyra carries the name that was almost Ziva's. Disclosed by the Architect 2026-06-09; canonical — recorded here, in `CLAUDE.md` §Persona, and `PERSONA_REGISTRY.md`.
