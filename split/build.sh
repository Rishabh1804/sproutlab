...
<head>
  ...
  <!-- PWA installability (2026-09-18): Chrome offers "Install app" only when the
       page links a manifest; iOS reads the apple-* tags for Add to Home Screen.
       These were carried by the retired beta/ pages and never made it into the
       split-file head — regression guard: tests/e2e/pwa-head.spec.ts. The
       theme-color value is the light-theme paper token; core.js toggleDarkMode
       rewrites it at runtime (#1a1a2e dark / #fdf0f3 light). -->
