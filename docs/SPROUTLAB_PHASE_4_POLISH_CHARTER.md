...
**2026-05-02 — Polish-10 reopen** (post-Polish-8 merge / supersedes PR-32 close):

Polish sub-phase formally closed at PR-32 (Polish-8 close artifact, merged); Stability sub-phase was the next planned cycle. Sovereign surfaced two visible Polish-grade Medical-tab artifacts (Food Reaction Timeline + Illness Frequency renderers). Both artifacts share a single root cause — `zi()`-rendered SVG markup stored as data on `ep.emoji` fields and concatenated into HTML `title="..."` attributes, where the embedded `class="zi"` quote prematurely terminates the attribute and leaks the trailing markup as rendered DOM text. Sovereign chose **broadest scope** + **clean architectural fix** (strip SVG from data; render at boundary). Polish-10 reopens the sub-phase to land that fix plus deferred-from-Polish R-10 hygiene queue items P-1 (HR-1 emoji audit ~33 sites) and P-6 (HR-3 settings-toggle delegation) plus broader HR-3 onclick sweep surfaced during Polish-10 scout. Polish-10 decomposes per R-9 split-threshold into Polish-10a / 10b / 10c (feature) + Polish-10 close artifact. Stability sub-phase handoff defers until Polish-10 close artifact merges. PR-32's "Polish closes on PR-32 merge" framing is superseded; reconciliation lands in Polish-10 close artifact.
...
### Polish-9 — Atomic-canon rename: simple-mode → essential-mode (Sovereign-via-Aurelius mid-sub-phase insertion)

**Sovereign-issued directive at mid-sub-phase** (post-Polish-1 merge): rename `simple-mode` → `essential-mode` across the codebase to align internals with already-existing user-facing "Essential Mode" display strings (core.js:4626 + :4672 pre-rename). Vocabulary-parity-between-UI-and-source first-instance (watch-list per Lean-Machine §B; no counter at first-instance).
...
**Files:** `split/core.js` (canonical surface: function defs `isSimpleMode/toggleSimpleMode/initSimpleMode → *EssentialMode`, classList ops, localStorage key `'ziva_simple_mode' → 'ziva_essential_mode'`, boot migration block at `initEssentialMode`, state-snapshot prop `.simpleMode → .essentialMode`, comment drop at :3700 obsoleted parenthetical), `split/start.js` (boot call), `split/home.js` (vitals gate + alert filter), `split/diet.js` (5 trends-modifier gates), `split/medical.js` (Polish-1 gates at :5805 + :5921), `split/template.html` (4 comments + settings toggle id + onchange function name; HR-3 inline-handler defect persists per Polish-9 scope-limit ratification, routed to charter §6 P-6 entry), `split/styles.css` (33 `body.simple-mode` CSS class refs). Total source: ~80-100 LOC.

**Boot migration (idempotent + race-safe):** read `'ziva_simple_mode'` → if value present + new key not already set → write `'ziva_essential_mode'` = old value → delete old key. Re-run is no-op (old=null after first migration). Multi-tab race safe (atomic per-key writes; B's clobber-guard skips if A already wrote new). Storage-cleared falls through to default-on logic.
...
**Tests:** R-7 triad covering migration contract (positive — fresh user defaults to essential-mode ON; regression-guard — boot migration preserves existing user state through key transition; positive-regression — idempotency across N runs + multi-tab race clobber-guard + storage-cleared default-on). Existing essential-mode contract assertions (2a/2b/2c smoke spec block) renamed in-place for the new body-class + key vocabulary; verify external-contract preservation. ~75 test LOC net change.

**Hold-pending-Sovereign-real-device-verification:** **applies** per RATIFIED hermetic-floor doctrine (3/3) — multi-device verification: Sovereign device + Bhavna device both run boot migration cleanly; existing toggle state preserved through key rename; cross-device toggle (if applicable post-rename) reflects correctly.

### Polish-10 (Sovereign-extended scope; charter-amendment 2026-05-02) — SVG-in-data architectural fix + broad HR-1/HR-3/HR-4 sweep

Polish-10 absorbs the Sovereign-surfaced visible-bug fix plus deferred-from-Polish R-10 hygiene items (P-1 HR-1 emoji audit + P-6 HR-3 settings-toggle delegation) plus the broader HR-3 onclick sweep that the Polish-10 scout surfaced. R-9 split-threshold demands decomposition: Polish-10 ships as **Polish-10a / 10b / 10c (feature) + Polish-10 close artifact**.

#### Polish-10a — Visible-bug architectural fix + HR-1 emoji sweep + HR-4 escAttr gaps + HR-2 niceties

**Files:** `split/medical.js` (data-shape change at illness-episode constructors `:6932-6935` + 5 render-site fixes + 4 HR-1 emoji + escHtml/escAttr application), `split/intelligence.js` (5 HR-1 emoji + 2 alert-icon data-field icon-key conversions + 2 escAttr title-attr gaps), `split/home.js` (1 HR-2 inline-style nicety + 2 escAttr title-attr gaps), `split/core.js` (3 HR-2 inline-style niceties), `split/styles.css` (`.mi-legend-item white-space: nowrap` + new utility classes), `tests/e2e/smoke.spec.ts`. ~25 sites total.

**Architectural shift:** SVG markup (output of `zi()`) MUST NOT cross a data-attribute boundary. Existing `escAttr` (quote-only escape) is insufficient when payload contains `<svg class="zi">` because the embedded `"` closes the surrounding `title="..."` prematurely. Existing `escHtml` (full HTML entity escape) would render the SVG as visible markup text — wrong fix. **Correct shape:** strip SVG-rendered fields from data structures; store only the icon-key string (`'flame'`, `'diaper'`, `'siren'`); render `zi(key)` at the display boundary (component innerHTML), separate from any `title="..."` text payload (which carries plain-text only, escaped via `escHtml` for defense-in-depth even when enum-controlled).

Cross-jurisdiction (Maren on Care surfaces; Kael on Intelligence surfaces; both on shared module styles.css).

**Tests:** R-7 triad — positive (Food Reaction Timeline + Illness Frequency Timeline render without `<svg` substring inside any `title=` attribute); regression-guard (grep-against-built-bundle assertion: pattern `title="' + .*\.emoji` must have zero matches in `sproutlab.html`); positive-regression (legend item still renders as `.mi-legend-item` with full label text). +25-40 test LOC.

#### Polish-10b — HR-3 onclick batch (Care + Home domain)

**Files:** `split/medical.js` (~12 onclick → data-action conversions across symptom-chips, doctor cards, vacc UI), `split/home.js` (~25 onclick → data-action across home-tab various — vitals, food cards, activity tracking, milestone surfaces), `split/core.js` (event-delegation registration), `tests/e2e/smoke.spec.ts`. ~30 sites total.

Maren-jurisdiction-primary; cross-Governor symmetric-catch eligibility on shared `core.js` delegation registrations.

**Tests:** R-7 binary-mode duos per batch (per PR-18 precedent — bug-IS-the-absent-state shape for handler-delegation completeness); positive (every converted site dispatches via delegated handler) + regression-guard (no `onclick=` substring at the converted line ranges). +30-50 test LOC.

#### Polish-10c — HR-3 onclick batch (Intelligence + Diet domain)

**Files:** `split/intelligence.js` (~17 onclick → data-action across Sleep Intelligence, Meal Balance, illness-episode trackers, kc-row-flex-wrap consumers), `split/diet.js` (~10 onclick → data-action across food logging, nutrition surfaces), `split/core.js` (~2 storage-display onclick if not already addressed in 10a), `tests/e2e/smoke.spec.ts`. ~30 sites total.

Kael-jurisdiction-primary; Maren reviews diet.js portion.

**Tests:** R-7 binary-mode duos per batch; same shape as Polish-10b. +30-50 test LOC.

#### Polish-10 close artifact

`docs/SPROUTLAB_PHASE_4_POLISH_CLOSE.md` amendment **OR** new sibling `docs/SPROUTLAB_PHASE_4_POLISH_10_CLOSE.md` (disposition lands at close-artifact author-time). Contents: Polish-10 reopen rationale + 10a/10b/10c per-PR close index + R-4 cumulative stress matrix floor advance from 9,867 + native ratification candidates surfaced (`svg-markup-must-not-cross-data-attribute-boundary`, `screen-bug-shares-root-with-other-render-paths`) + PR-32 close-artifact reconciliation note (Polish actually closed at Polish-10-close, not PR-32) + Sovereign verification stack delta + Stability sub-phase handoff (actually opens here).

Pure docs (Edict VIII parallels Polish-charter and Polish-8 close).
...
13. **Phase 4 R-10 carry-forward documented and routed.** The full HR-1 emoji audit (Cipher PR-23 r1 Catch 3 surfaced ≥18 emoji-bearing rendered-UI sites; Polish-2 closes the 2 named cross-Governor-catch sites; ~16 remaining: home.js weather-code emoji map at `:253-257` + precautions strings at `:299-323` + weather advisory at `:336/:339` + food icon props at `:1115/:1190`; medical.js doctor avatar/modals at `:2355/:2385/:2401` + activity icon props at `:3914/:3930` + symptoms-render text at `:6521`) routed to Stability sub-phase or dedicated R-8 charter on-record per `architectural-surfacing-must-enumerate-axis-of-resolution`. **Amendment 2026-05-02:** P-1 partially absorbed into Polish-10a (the Care-jurisdiction subset + intelligence cross-Governor sites identified at Polish-2 r1).

14. **(Amendment 2026-05-02 — Polish-10)** **SVG-in-data architectural shift completed.** Zero `zi()` SVG-rendered output stored as data on any `ep.emoji`-shaped field across `medical.js`. Icon-key strings stored instead; `zi()` rendered at display boundary; `title="..."` attributes carry plain-text only (escHtml-applied for defense-in-depth). Two visible bugs surfaced by Sovereign in Medical → Info → Food Reaction Timeline + Medical Intelligence → Illness Frequency closed. Verified via R-7 triad including grep-against-built-bundle regression guard.

15. **(Amendment 2026-05-02 — Polish-10)** **HR-3 onclick sweep landed.** Approximately 60 inline `onclick=` handlers across home.js / medical.js / intelligence.js / diet.js / core.js converted to `data-action` delegated handlers per Polish-2 established convention. P-6 (settings-toggle delegation, `template.html:2594`) closed. Verified via grep-against-built-bundle regression guard: zero `onclick=` substrings remain at converted line ranges.

16. **(Amendment 2026-05-02 — Polish-10)** **Polish-10 close artifact published** at `docs/SPROUTLAB_PHASE_4_POLISH_CLOSE.md` (amended) or sibling close doc, including: PR-32 close-artifact reconciliation note, R-4 cumulative floor advance from 9,867 across 10a/10b/10c, native ratification candidates surfaced, Sovereign verification stack delta, and Stability sub-phase actual-open declaration.
...
|---|---|---|---|
| P-1 | **Full HR-1 emoji audit (~33 remaining sites; was ~16 at PR-23 r2 → ~22 at Polish-2 build-deep → ~33 at Polish-2 r2 with Cipher self-acknowledged audit-coverage gap)** in home.js weather-code emoji map (`:253-257`, 5 sites), precautions strings (`:299-323`, 6 sites), weather advisory templates (`:336/:339`, 2 sites), food icon props (`:1115/:1190`, 2 sites); medical.js doctor avatar/modal text (`:2355/:2385/:2401`), activity icon props (`:3914/:3930`), symptoms-render text (`:6521`); **medical.js:2255 severity label** (`'🚨 Emergency'` — Polish-2 build-deep find); **intelligence.js cross-Governor-territory finds** at `:7886, :7898, :8083, :8200, :8573` (5 🩲 diaper-emoji sites in care-tracker pictograms — Cipher self-acknowledged at Polish-2 r1 review), `:11828 + :11889` (2 additional 🩺 stethoscope sites — Cipher self-acknowledged), `:11958 + :11965` (severity-label + emergency-care-callout duplicates of medical.js:2255 + :2268 — DRY violation, Polish-2 build-deep find), `:54710 + :54771` (🩺 in Symptom Checker chip + modal title); **template.html shared-module finds** at `:1339 (🩸), :1340 (🫧), :2861 (🩺), :2887 (🪔), :11681 (🩺 Add Doctor modal title)` — Cipher self-acknowledged. Fix-shape varies materially: simple `zi()` swap for some (doctor modal text, symptoms-render); structural refactor of icon-props/data-structures for others (weather-code emoji map needs code→sprite-name table with `zi()` resolution at render time; food icon props ditto; care-tracker pictograms ditto); DRY-refactor candidate for the duplicated severity-label string between medical.js:2255 + intelligence.js:11958 (same `'\u{1F6A8} Emergency'` literal in both files — promote to a shared helper); shared-module template.html sites need static→dynamic conversion to use `zi()` rendering. | Cipher PR-23 r1 Catch 3 deeper audit (initial ~16; Care-jurisdiction-scoped) + Polish-2 build-deep audit (added 6 sites; surfaced cross-Governor-catch coverage gap) + Polish-2 r1 Cipher self-acknowledged audit-coverage gap (added 11 sites in Kael's territory + shared module that Cipher's r1 missed; running-beats-reading-from-Cipher's-side 6th instance per `Codex/docs/doctrine-ledger.md`) | **Amendment 2026-05-02:** Polish-10a partially absorbs (medical.js `:2355, :2385, :2401, :6533` 🩺 sites + intelligence.js `:7886, :7898, :8083, :8200, :8573, :11828, :11889` 🩲/🩺 sites + alert-icon data-field icon-key conversions). Remaining sites (home.js weather-code emoji map data-structure, precautions strings, food icon props; medical.js activity icon props at `:3914/:3930`; intelligence.js Symptom Checker `:54710/:54771`; template.html shared-module sites; DRY-refactor for duplicated severity literal) **continue to defer to Stability sub-phase or dedicated R-8 charter** — fix-shape variability not in Polish-10 scope. |
...
| P-6 | **HR-3 settings-toggle delegation** — `template.html:2594` carries `<input id="settingsEssentialMode" onchange="toggleEssentialMode()">` — inline `onchange` handler is an active HR-3 violation. Polish-9's atomic-canon scope (vocabulary rename) renamed the function name in the inline handler but **the HR-3 violation persists**. Convert to `data-action="toggleEssentialMode"` with delegated handler. ~5 LOC source. | Polish-9 scout-deep finding (Sovereign-via-Aurelius ratified disposition: NOT in Polish-9 scope; route to R-10) | **Amendment 2026-05-02:** ABSORBED into Polish-10b (Care + Home HR-3 onclick batch). Item closes at Polish-10b merge. |
| P-5 | **Defense-in-depth JS-layer gating audit + sweep of remaining ~19 medical.js `renderInfo*` renderers** (renderInfoPoopConsistencyTrend, renderInfoPoopFrequency, renderInfoPoopColorAnomaly, renderInfoPoopAmountTrend, renderInfoPoopSymptoms, renderInfoPoopReport, renderInfoActivityConsistency, renderInfoMilestoneVelocity, renderInfoActivityCorrelation, renderInfoIllnessFreq, renderInfoVaccFever, renderInfoIllnessFood, renderInfoRecovery, renderInfoIllnessSleep, renderInfoFoodReaction, renderInfoRepetition, renderInfoTexture, renderInfoHydration, plus any new-Info-renderer drift). All write into Info-tab DOM that is CSS-hidden in simple-mode via `body.simple-mode .tab-btn[aria-label="Info tab"] { display:none; }`. Polish-1 lands the discipline at 2 named sites (renderInfoPoopFoodDelay + renderInfoPoopFoodWatch); P-5 carries the sister-shape sweep for the remaining 19. | Lyra build-deep catch at Polish-1 author-time (running-beats-reading sister instance to Cipher PR-23 r1 Catch 3); Sovereign-via-Aurelius Path-1 ratification | **Stability sub-phase candidate** — address if Sovereign deems perf/defense-in-depth worth pursuing post-Polish. Charter explicitly defers per Path-1 ratification. |
| P-7 | **Cross-jurisdiction `.section-caption` cousin-pattern sweep** — empirical Polish-5 build-deep grep finds **44 sites** (home.js 6 + diet.js 11 + medical.js 13 + intelligence.js 14 + core.js 0) matching the cousin pattern `style="font-size:var(--fs-{xs,sm,base});color:var(--{light,mid});margin-{bottom,top}:Xpx;"` with significant variation: different font-sizes (xs / sm / base), different colors (light / mid), different margins (4 / 5 / 6 / 8 / 10 / 12 px), various combinations with `padding`, `width`, `text-align`. Charter cite was ~34 sites; running-beats-reading 9th instance count drift. Polish-5 narrow-scope discipline (per RATIFIED 3/3 narrow-scope-and-defer-broader-audit-to-R-10 doctrine) defers this multi-variant sweep to a deliberate class-taxonomy decision: a single `.section-caption` class can't cover all 44 variants without either (a) collapsing to multiple sub-classes (taxonomy bloat) or (b) lossy collapse (visual margin shift via spacing token). | Lyra Polish-5 build-deep finding (running-beats-reading 9th instance) | **Stability sub-phase candidate** OR dedicated Polish-N class-taxonomy charter — address if Sovereign deems class-taxonomy normalization worth the design-decision investment. |

R-10 threshold (3–5 items) **EXCEEDED at 7 items at Polish-5 scout-deep** (was 6 post-Polish-9; P-7 added per Polish-5 narrow-scope decision; running-beats-reading 9th instance count drift). Disposition: items P-1 + P-2 + P-5 explicitly route off the Polish sub-phase per their listed dispositions; items P-3 + P-4 close in-Polish-build; **P-6 is Polish-2-expansion-or-dedicated-Polish-N candidate** (close before Polish sub-phase final). With queue at 6 (over R-10's 3–5 band), a hygiene-sweep PR consolidating P-3 + P-4 + P-6 in-Polish-build is now warranted; framing of that consolidation lands at Polish-N close-time when build-state is concrete.
...
**Amendment 2026-05-02 — Polish-10 R-10 status:** P-1 (partial) + P-6 (full) absorb into Polish-10. P-1 carries forward in reduced form (sites listed in §6 P-1 disposition column). P-2 / P-3 / P-4 / P-5 / P-7 dispositions unchanged.
...
**Amendment 2026-05-02 — Polish-10 sequencing:**
- Polish-10a is independent of 10b/10c (different fix-shapes; data-shape change vs handler delegation). 10a should land first to discharge the visible-bug fix on Sovereign's verification stack.
- Polish-10b and 10c both touch `core.js` event-delegation registration; merge-conflict risk if cut in parallel. Sequence: 10b → 10c.
- Polish-10 close artifact lands after 10c merge; depends on all three.
- Hour-budget: Polish-10a ~1.5 days (architectural shift + R-7 triad authoring); 10b ~1 day (mechanical conversion ×30); 10c ~1 day (mechanical conversion ×30); close artifact ~0.5 day. Total ~4 days at Lyra cadence.
