...
**Folds:** issue [#54](https://github.com/Rishabh1804/sproutlab/issues/54) (closed via Phase 2 fold; contract clause **PC-1**) · issue [#57](https://github.com/Rishabh1804/sproutlab/issues/57) (open; Class C sequencing surfaces in **PC-7**)
...
| **PC-2.4** | Chip body text node interpolation MUST use `escHtml(text)` (HTML context). Chip attribute interpolation containing user text MUST use `escAttr(escHtml(text))` double-wrap until issue #57 lands the global `escAttr` fix; thereafter the contract amends to single-wrap `escAttr(text)` per #57's acceptance criteria. | **MAJOR** (escape contract is load-bearing for AT pronunciation + XSS hygiene) | grep: every interpolated `${escAttr(...)` inside chip / chip-x markup MUST sit inside a corresponding `escHtml(...)` wrap (or be a known-Class-A invariant per #57's census). Reviewer manual inspection per the PC-2 catalog escape contract. | cheap (rule codifies existing v6.2 §4 idiom) | Class A inputs (slug-IDs, controlled-vocabulary names) per #57's census MAY skip the `escHtml` inner wrap; Class A status MUST be cited in the PR description. PR-ε.0 v6.2's chip × `aria-label` (`escAttr(escHtml(labelText))`) is canonical. |
...
- **escAttr global-fix interaction.** PC-2.4's double-wrap requirement is explicitly time-bound: until #57 lands path (a) per the Class A+B / Class C path-split (see PC-7.3), the double-wrap is mandatory. Post-#57, the contract amends to single-wrap. Phase 4 / Phase 5 hygiene cycles handle the simplification.
...
| **PC-7.3** | Inline `onclick=` interpolations of `data-action`-class strings (e.g. `home.js:8369` `onclick="${item.action}"`) MUST migrate to `data-action` attributes routed through the dispatcher. The 3 Class C sites (`home.js:8010 / :8118 / :8168` per Phase 2's #57 census) sequence through this clause. | **MAJOR** (sequencing — Class C in #57 cannot land path-(a) `escAttr` rewrite until PC-7.3 closes) | grep: `grep -cE 'onclick="\$\{' split/home.js` MUST equal 0; the consumer call site (`home.js:8369`) MUST resolve to `<button data-action="…">` element-of-record. | medium (3 builder sites + 1 consumer) | None for new code. Pre-existing 3 Class C sites are the sequencing target; #57 path (a) lands globally only after PC-7.3 closes. |
...
| **PC-7.5** | `data-arg` / `data-arg2` / `data-id` interpolations carrying user-text MUST use `escAttr(escHtml(text))` double-wrap (until #57 lands) or the post-#57 single-wrap. Class A inputs (slug-IDs, controlled-vocabulary names) MAY skip the inner `escHtml` per #57's census. | **MAJOR** (escape contract; identical to PC-2.4 in shape) | grep: every `${escAttr(...)` interpolation inside `data-arg*` / `data-id` MUST sit inside an `escHtml(...)` wrap, or be Class A and cited as such in the PR. | cheap | Class A inputs per #57 census are exempt and MUST be cited. |
...
5. **PC-7.1 dispatcher consolidation severity.** I marked PC-7.1 as **MAJOR**. Cipher should challenge whether the `expensive`-cost migration justifies a MINOR severity (consolidating four ladders into one is hygiene; nothing breaks at audit-base from the ladder count). My read: MAJOR because the four-ladder design is a debugging surface that compounds with every feature; severity here is not gap-driven but architecture-driven. Cipher should weigh in on whether architecture-driven severity belongs in this contract or in a separate architectural-decision document.
