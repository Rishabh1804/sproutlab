...
Read `docs/specs/food-sub-tab-v1.md` §F-3. F-3 = relocate `home.js:~4690 renderFoods` + `:~4782 renderFoodCatSubContent` into the Diet Log sub-tab as a proper food-DB browser (search + filter + nutrition cards). Surface a concrete IMPL plan via `AskUserQuestion` and get Architect ratification **before** building (per the F-arc pattern from prior sessions).

### Priority 2 — F-3 IMPL → canon-cc-008 chain
Build (likely staged commits), run `/code-review xhigh` if substantial (AGENTS.md Rule 13), then the canon-cc-008 chain. Diff will likely touch `home.js`/`diet.js` (Maren), `intelligence-quicklog.js` or render surfaces (Vela), `data.js` (Kael consult), and probably `styles.css`/`template.html` (triple-Gov). Lyra fold-authority on food-sub-tab findings is standing. Cipher Edict V terminal pass (CV3-006 three-axis).
...
>
> **Session goal:** Open food-sub-tab-v1 **F-3 (Library consolidation)** — relocate the food-DB browser (`home.js` `renderFoods` / `renderFoodCatSubContent`) into the Diet Log sub-tab with search + filter + nutrition cards, per `docs/specs/food-sub-tab-v1.md` §F-3. Spec-read → ratify scope via AskUserQuestion → IMPL → `/code-review xhigh` if substantial → canon-cc-008 chain → Cipher Edict V → mark ready/merge.
>
