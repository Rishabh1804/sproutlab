...
function _predictMilestoneWindow(milestoneId, opts) {
  ...
  // via the timezone-safe helper.
  const dateForAge = opts.dobOverride ? toDateStr(new Date(opts.dobOverride)) : today();
  const nowDays = _zivaAgeInDays(today());
  const ageDays = nowDays;
