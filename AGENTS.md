...
9. **SproutLab — Governor audit chain before merge (canon-cc-008; post-canon-gen-001 routing).** Every Capital change (edit under `split/`) clears the Governor→synthesis→Cipher chain before its PR leaves draft or merges. Routing by diff:
   ...
   - touches `diet.js` / `recipes.js` → **Ceres** (Nutrition — canon-gen-001 second-generation; seated 2026-06-08)
   - touches `intelligence-isl.js` / `intelligence-qa.js` / `intelligence-qa-handlers.js` / `intelligence-illness.js` / `intelligence-correlate.js` / `intelligence-caretickets.js` / `core.js` / `data.js` / `sync.js` / `config.js` / `start.js` → **Kael** (Intelligence engine)
   ...
   - touches `styles.css` / `template.html` → **all four** in sequential quadruple-jurisdiction (rotation Maren → Ceres → Kael → Vela, first-Governor by heaviest-touched Region)
