...
**PR Tree Dashboard:** [docs/PR_TREE_DASHBOARD.html](docs/PR_TREE_DASHBOARD.html) — interactive sunburst + collapsible tree of every SproutLab PR, each carrying what-it-was / what-it-achieved / what-it-solved. Regenerated each build by `split/build-pr-dashboard.mjs` from `docs/pr-dashboard-data.json` (the curated source of truth — the build is offline, so records are appended by hand, not scraped) + `split/pr-dashboard-template.html`; the `.html` is a VIEW with a don't-hand-edit banner. **When a PR merges, append its record to the data file** (number, title, date, merged, what, achieved, solved, category, kind — category from the fixed taxonomy inside the template's `CAT_COLORS`) so the next build folds it in.
