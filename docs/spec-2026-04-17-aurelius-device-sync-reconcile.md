...
7. **KL-2 (C0 v3.1 residual — Cipher, 17 April 2026): Dotted-named literal top-level fields from pre-fix pushes persist in Firestore.** Pre-B4-fix pushes created literal top-level fields with dots in their names (e.g., `ziva_feeding.2026-04-17`) alongside the intended nested-path map. These orphaned fields remain after the B4 fix ships. Harmless to sync (listener iterates lsKeys, not all fields; dotted-literal fields are ignored by the key-matched read). Contribute to document size toward Firestore's 1 MiB limit, but static — they will not regrow once the fix is live. Manual cleanup via Firebase Console is optional per-document. Identifier pattern to delete: any top-level field whose name contains a `.` character.
