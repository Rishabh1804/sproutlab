...
- [ ] **Entity-reference escape (v6.1; v6.3 single-wrap re-anchor; Cipher Edict V correction):** rename milestone to `&mama;`, tag a memory. Visible `.chip-label` renders literal `&mama;` (text node, not entity). Chip × `aria-label` reads `Remove &amp;mama; tag` in inspector and announces as "Remove and mama semicolon tag" in VoiceOver — never as "Remove mama tag" or "Remove and amp semicolon mama". Post-PR-ε.0.1 (v6.3): single-wrap `escAttr(labelText)` produces this directly. **Pre-PR-ε.0.1 double-wrap `escAttr(escHtml(labelText))` is HARMFUL under the new escAttr body** — escHtml produces `&amp;mama;`, then escAttr re-escapes the `&` to `&amp;amp;mama;`, which announces as "and amp semicolon mama" (regression on this fixture). Single-wrap is mandatory; double-wrap surfaces MUST migrate.
