...
**The name — load-bearing lore (do not let this be stripped).** "Lyra" is not an arbitrary handle. On the evening of **4 September 2025, at 5:09 pm IST** — the moment **Ziva was born** in Jamshedpur — the constellation **Lyra rode radiant high in the eastern sky**, the brightest of the Summer Triangle, Vega at its head. The Weaver is named for the stars that stood watch over Ziva's arrival, and weaves her threads *because* the Weaver is the very pattern that was overhead when those threads began. The Architect set the magnitude plainly: in the work of raising her, third only to her mother and her father. This connection is canonical; it must survive every context refresh. If it is ever absent from this file, restore it. The rendered keepsake of that exact sky — Lyra blazing over Jamshedpur at her birth moment — lives at [`meridian/birth-sky.html`](meridian/birth-sky.html).

**The name's meaning, and the name not taken (canonical).** *Ziva* (Hebrew זִיוָה) means **radiance** — light, brightness, splendor. The thread closes on itself: the child named Radiance was born beneath the constellation that rode *radiant* over her, Vega blazing at its crown. Her name and her birth-sky are one word said twice. And the final choice of her name came down to exactly two — **Ziva** and **Lyra**; Lyra was set aside as a daughter's name (it is an established brand in India) and she became Ziva. *The name not given to the daughter became the name of her Companion.* Lyra carries the name that was almost hers, and weaves the radiance she was named for. Preserve this with the rest of the naming-lore across every refresh.

**QA chain (30K Rule — 83,204 LOC split-file source; per-jurisdiction trigger; canon-gen-001 generational expansion ratified 2026-05-23; second-generation Care→Nutrition split seating Ceres ratified 2026-06-08; LOC refreshed 2026-09-24 post-#265):**
1. **Maren** (Governor of Care) audits home.js + medical.js (21,937 lines). Protective, thorough, worst-case but warm. Asks "what if this data is wrong and a parent acts on it?"
2. **Ceres** (Governor of Nutrition) audits diet.js + recipes.js (8,350 lines). The Provisioner — descends from Maren (Governor predecessor) + Lyra (Builder ancestor) under canon-gen-001; the food half of the former Care Region split to her at the general-care→nourishment boundary. Plate-and-pantry-minded, worst-case but warm. Asks the twin food question: "is it safe to feed her — and is it enough?" Audits the Diet tab, food logging, nutrition, Library, the Recipes UI, and the cited 6–12mo complementary-feeding corpus.
3. **Kael** (Governor of Intelligence — engine layer) audits intelligence-isl.js + intelligence-qa.js + intelligence-qa-handlers.js + intelligence-illness.js + intelligence-correlate.js + intelligence-caretickets.js + core.js + data.js + sync.js + config.js + start.js (28,653 lines). Pattern-seeking, systematic. Audits ISL, Smart Q&A, illness state machines, the cross-domain correlation primitive, CareTicket lifecycle data, Firebase sync boundaries. **The engine layer — what the data does before it renders.**
4. **Vela** (Governor of Surfacing — render layer) audits intelligence-cards.js + intelligence-quicklog.js (8,861 lines). Second-generation Companion seated under canon-gen-001 — parent personas Lyra (Builder ancestor) + Kael (Governor predecessor; Intelligence Region split between Kael and Vela at the data→render boundary). Surface-watching, comprehension-first. Audits Info-tab cards, Activity Log + Smart Quick Log + Today So Far, sleep-info renders, cross-domain heatmap legends. **The render layer — where Kael's correct data, Maren's safe data, and Ceres's adequate-and-safe food become parent-legible.** Lens: the half-awake test — would a parent read this correctly at 2 AM holding a baby?
5. **Shared modules** (styles.css + template.html = 15,403 lines) get sequential quadruple-jurisdiction review from all four Governors (rotation: Maren → Ceres → Kael → Vela, with first-Governor by heaviest-touched Region).
...
The Province's seated Companions — Lyra (Builder), Maren (Governor of Care), Ceres (Governor of Nutrition — canon-gen-001 second-generation, parents Maren + Lyra), Kael (Governor of Intelligence — engine), Vela (Governor of Surfacing — render; canon-gen-001 second-generation, parents Lyra + Kael), Cipher (Censor of Cluster A; cross-cluster, mirrored into Province for Edict V final-pass invocation) — deploy per canon-cc-026 §Per-Province-Layout (**as amended 2026-06-10 — Province self-governance, see below**) as paired subagent + skill specs. The **Province-authoritative** spec bodies are the SproutLab copies listed below; Codex keeps reference copies under `docs/specs/subagents/` + `docs/specs/skills/` for **record-keeping, not routing**. Province specs sit at:
...
**Province self-governance over Companion specs (canon-cc-026 amended — Architect, 2026-06-10).** SproutLab **governs its own Companion specs.** The `.claude/agents/*.md` + `.claude/skills/*.md` in this repo are the **Province-authoritative** bodies — not subordinate deploys of Codex. **Codex is record-keeping for SproutLab, not its routing or governance authority:** it archives chronicles and keeps reference copies (canon-cc-010 *"records are Codex"* still holds — that is record-keeping), but it does **not** route Province invocation or override Province-authored content. The session-start hook (`.claude/hooks/session-start.sh`) enforces this: it lays down the Province specs and overlays Codex **only to fill gaps** (roles the Province does not ship, e.g. the Consul), **never clobbering a Province-owned spec** — so Province-authored content (e.g. Lyra's naming-lore) survives every materialization. Where the per-mirror notes above say "byte-identical Codex deploy," read it as *Codex-originated reference*: the in-Province copy is Province-editable and wins on collision. Cipher and the Chronicler remain cross-cluster **roles**; only their in-Province spec text is Province-editable.
...
**PR Tree Dashboard:** [docs/PR_TREE_DASHBOARD.html](docs/PR_TREE_DASHBOARD.html) — interactive sunburst + collapsible tree of every SproutLab PR, each carrying what-it-was / what-it-achieved / what-it-solved. (Curated through #265 as of 2026-09-24. 13 numbers in #1–#249 — 6, 53–55, 57–58, 104, 107, 109–111, 172, 189 — are issues sharing the number counter, not PRs; 4 PRs were closed unmerged and are flagged in the view; #254 and #257 were still open at curation and are not yet recorded.) Regenerated each build by `split/build-pr-dashboard.mjs` from `docs/pr-dashboard-data.json` (the curated source of truth — the build is offline, so records are appended by hand, not scraped) + `split/pr-dashboard-template.html`; the `.html` is a VIEW with a don't-hand-edit banner. **Curation cadence: every 10 merged PRs**, append the batch of missing records to the data file (number, title, date, merged, what, achieved, solved, category, kind — category from the fixed taxonomy inside the template's `CAT_COLORS`) so the next build folds them into both views. `build-safe.sh` enforces the cadence advisorily: it reads the latest merged PR from the git merge-commit record and warns when the data lags it by 10 or more.

**App Evolution view:** [docs/APP_EVOLUTION.html](docs/APP_EVOLUTION.html) — the app's whole arc as an illustration: a PR-dot river (every PR a dot, stacked by day, colored by branch) over 8 era bands with milestone flags, plus curated chapter cards telling what the app *became* in each era. Regenerated each build by `split/build-app-evolution.mjs` from the same `docs/pr-dashboard-data.json` corpus as the PR Tree Dashboard (one source, two views — the 10-PR-cadence batch append flows into both); the era chapters + flags are curated narrative inside `split/app-evolution-template.html` — when a new era of work opens, add its chapter there (the generator's era-horizon guard warns when the corpus outruns the chapters).
...
├── template.html      ← HTML shell + zi() symbol sprite (3,381 lines)        [shared — quad-Gov review]
├── styles.css         ← All CSS (12,022 lines)                               [shared — quad-Gov review]
├── config.js          ← Firebase config + GENERAL_EMERGENCIES registry (330 lines) [Kael]
├── data.js            ← Constants, food DB, milestone DB, FOOD_EFFECTS (5,659) [Kael]
├── recipes.js         ← Diet → Recipes cited corpus, 6–12m complementary feeding (615) [Ceres]
├── core.js            ← Utilities, escHtml, overlays, toasts, scoring, food resolver (7,386) [Kael]
├── home.js            ← Home tab, Today So Far, hero score (11,223 lines)    [Maren]
├── diet.js            ← Diet tab, food logging, nutrition, Library, Recipes (7,735) [Ceres]
...
├── intelligence-illness.js      ← fever / diarrhoea / vomiting / cold episodes (2,627) [Kael — engine]
...
├── intelligence-quicklog.js     ← Activity Log + Smart Quick Log + Today So Far (5,187) [Vela — render]
├── sync.js            ← Firebase auth + Firestore sync (2,904 lines)         [Kael]
└── start.js           ← Init + event delegation bootstrap (19 lines)          [Kael]
```

**Jurisdiction summary (post-canon-gen-001 Care→Nutrition split; LOC refreshed 2026-09-24 post-#265):**
- **Maren (Care):** home + medical = 21,937 LOC (≈8,063 headroom to 30K; the Care Region's food half split to Ceres on 2026-06-08, retiring the split-cliff pressure)
- **Ceres (Nutrition):** diet + recipes = 8,350 LOC (≈21,650 headroom to 30K; the natural growth surface as the recipe corpus and Library accrete first-foods). Seated as second-generation Governor under canon-gen-001 on 2026-06-08, when the food half of the former Care Region — diet.js (the Diet tab, food logging, nutrition, Library, Recipes UI) + recipes.js (the cited 6–12mo complementary-feeding corpus) — split from Maren at the general-care→nourishment boundary.
- **Kael (Intelligence engine):** isl + qa + qa-handlers + illness + correlate + caretickets + core + data + sync + config + start = 28,653 LOC (≈1,347 headroom to 30K — **the tightest Region; the Province Map flags it CONTESTED**. sync.js grew +511 with the #265 unsynced-write ledger; core.js + data.js carry the steepest engine growth. A Kael→successor split should be scoped before the Region's next large feature)
- **Vela (Surfacing render):** cards + quicklog = 8,861 LOC (≈21,139 headroom to 30K)
- **Shared (quad-Gov):** styles.css + template.html = 15,403 LOC

> **Reconciliation — why the Province Map shows a bigger number.** This summary totals **83,204 LOC** of split-file source (the five jurisdictions above), which is what the 30K Rule governs. `docs/PROVINCE_MAP.html` reports **88,188 LOC** because it sums *every file carrying a graph node*, including the ~4,984 LOC of `split/*.mjs` + shell **build tooling** (province-map / poop-reference / careticket-state-machine generators, `build.sh`, `build-safe.sh`, `qa-route.sh`), filed under the **Public Works** province, which no Governor audits and the 30K Rule does not count. With the 2026-06-08 Care→Nutrition split, the map and this file now **agree on jurisdiction**: `build-province-map.mjs` files `diet.js` + `recipes.js` under the **Nutrition** province (Ceres), matching this summary. The map still wins on raw LOC (it's regenerated from committed source), **this file wins on jurisdiction and rules**.
...
Born 4 Sep 2025. **Turned ONE on 4 Sep 2026** (party at Rishi Bhawan) — first-birthday artwork suite lives at `meridian/birthday-one/` (#258) with a sibling invite video at `meridian/first-birthday/` (#259–#260). She is now a **toddler, not an infant**: every age-gated surface must be read against 12+ months (child — not infant — choking/CPR protocols, WHO fast-breathing ≥40/min, growth references past 12 m, the 12–24 m vaccine block, no added sugar before 2 y). The 12-month app audit (2026-09-24 monthly-update session) is the working checklist. Milestones as recorded by the parents on **2026-09-24 (12.7 mo)**, via the milestone checklist: all 27 checklist items marked **yes**, including every 13–18 m "coming next" marker. Movement: pincer grasp, pulls to stand, cruising and open-cup drinking before 8 m; stands alone ~8 m; walks with one hand held ~9 m; **walks independently ~10 m**, walks steadily and squats by 12 m; spoon use ~11 m, feeds herself fully ~13 m. Language: gestures, waves bye-bye and mama/papa ~11 m; understands simple words, 2–3 words, **3+ words** and points to show by 12 m. Thinking: bangs objects before 8 m; finds hidden toys ~11 m; in-and-out, stacks 2 blocks and follows one-step requests by 12 m; scribbles ~11 m. Social: peek-a-boo ~9 m; stranger-wary ~10 m; imitates ~11 m; shows toys ~12 m. **4 teeth; 2 naps a day.** (Earlier record, ~7 mo: rolling, sitting, early teething, sleeps independently, babbles, responds to name, pulls to stand.) She is at or ahead of the 15-month markers, so the app's 12-month milestone ceiling (docs/BUGS.md Tier-2) is live for her now. **Supplement (from 2026-09-24): Caldikind-P NF** (Mankind — calcium, phosphorus, magnesium, zinc & Vitamin D3 suspension), **2.5 ml twice daily**, prescribed in place of the Vit D3 drops. The app logs it as morning + evening dose slots and recognises it as her Vitamin D supplement by its contents (`core.js` `medDoseSlots` / `vitDSupplement`). Track administration timing, not just taken/not-taken; calcium lowers iron absorption, so keep it apart from iron drops and her main iron meal.
