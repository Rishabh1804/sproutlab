...
| **PC-2.4** | Chip body text node interpolation MUST use `escHtml(text)` (HTML context). Chip attribute interpolation containing user text MUST use single-wrap `escAttr(text)` — double-wrap (`escAttr(escHtml(text))`) is FORBIDDEN under the post-PR-ε.0.1 escAttr body. `escAttr` (`core.js:2565`) emits standards-compliant HTML-attribute escapes for `&` / `"` / `'`; double-wrapping causes `escAttr` to re-escape the `&amp;` produced by the inner `escHtml` into `&amp;amp;`, which mis-pronounces `&mama;` in VoiceOver as "and amp semicolon mama" instead of "and mama semicolon". | **MAJOR** (escape contract is load-bearing for AT pronunciation + XSS hygiene) | grep: every interpolated `${escAttr(...)` inside chip / chip-x markup MUST receive a single argument; `${escAttr(escHtml(...))}` MUST equal 0 across `split/*.js` + `split/template.html`. Reviewer manual inspection per the PC-2 catalog escape contract. | cheap (rule codifies post-PR-ε.0.1 v6.3 §4 idiom) | PR-ε.0 v6.3's chip × `aria-label` (`escAttr(labelText)`) is canonical. Pre-PR-ε.0.1 surfaces using double-wrap `escAttr(escHtml(text))` are now actively-failing on entity-bearing text — they MUST migrate to single-wrap. |
...
| **PC-7.5** | `data-arg` / `data-arg2` / `data-arg3` / `data-id` interpolations carrying user-text MUST use single-wrap `escAttr(text)` — double-wrap is FORBIDDEN under the post-PR-ε.0.1 escAttr body (same trace as PC-2.4: double-wrap re-escapes `&amp;` into `&amp;amp;`, breaking entity-reference correctness for any text containing `&`). `escAttr` (`core.js:2565`) emits standards-compliant HTML-attribute escapes for `&` / `"` / `'`. | **MAJOR** (escape contract; identical to PC-2.4 in shape) | grep: every `${escAttr(...)` interpolation inside `data-arg*` / `data-id` MUST receive a single argument; `${escAttr(escHtml(...))}` inside data-arg / data-id MUST equal 0. Reviewer manual inspection. | cheap (rule codifies post-PR-ε.0.1 v6.3 idiom) | Pre-PR-ε.0.1 surfaces using double-wrap on data-arg / data-id are actively-failing on entity-bearing text — Polish-class migration target with MAJOR severity (not "harmless redundancy" as an earlier draft of these notes incorrectly claimed). |
