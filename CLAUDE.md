...
**Model-tier doctrine (2026-07-27).** Companion invocation cost follows [`docs/MODEL_SELECTION_PROTOCOL.md`](docs/MODEL_SELECTION_PROTOCOL.md): Scribes run on pinned cheap tiers (scout/verify → Haiku 4.5, draft/record → Sonnet 5, via `model:` frontmatter); Builders, Governors, Cipher, and the Chronicler inherit the session model — **the canon-cc-008 QA chain never runs below it** (the safety floor; a pinned model is a ceiling as well as a floor). Skills always run in-session at the session model. Record copy in Codex per canon-cc-010.
...
**PR Tree Dashboard:** [docs/PR_TREE_DASHBOARD.html](docs/PR_TREE_DASHBOARD.html) — interactive sunburst + collapsible tree of every SproutLab PR, each carrying what-it-was / what-it-achieved / what-it-solved. (13 numbers in #1–#249 — 6, 53–55, 57–58, 104, 107, 109–111, 172, 189 — are issues sharing the number counter, not PRs; 4 PRs were closed unmerged and are flagged in the view.) Regenerated each build by `split/build-pr-dashboard.mjs` from `docs/pr-dashboard-data.json` (the curated source of truth — the build is offline, so records are appended by hand, not scraped) + `split/pr-dashboard-template.html`; the `.html` is a VIEW with a don't-hand-edit banner. **Curation cadence: every 10 merged PRs**, append the batch of missing records to the data file (number, title, date, merged, what, achieved, solved, category, kind — category from the fixed taxonomy inside the template's `CAT_COLORS`) so the next build folds them into both views. `build-safe.sh` enforces the cadence advisorily: it reads the latest merged PR from the git merge-commit record and warns when the data lags it by 10 or more.
