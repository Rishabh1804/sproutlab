...
- **No-time `urgent` spine-suppression (V-V-34 synth-fold) — RATIFIED 2026-05-28: option (a) promote.** `_tsfDaySpineSelect` currently selects from `allEvents` (timed) only; the no-time branch is hidden while the spine is collapsed. Architect ratified the closure: **no-time `urgent` events render as a synthetic header ABOVE the spine** (visually distinct; parent sees them but they don't compete with timed events for chronological sort position). Rationale: preserves the spine's "timed events ordered chronologically" contract while honoring the Honesty floor — parent never silently loses an urgent event. Options (b) include-in-pick-set and (c) require-synthetic-timeMin both ratified-rejected (b creates ordering ambiguity; c puts producer-side burden + fake-timestamp drift risk on the recommendation pipeline). v3-1 spec author implements this contract in the producer + the TSF header surface; v3-1 producer SHALL ship the synthetic-header render-path alongside the urgent producer wire-up.
